...
class MethodHasher {
    std::size_t operator()(const Method& method) const noexcept{
        return std::hash<std::string>{}(method.name)*13 + std::hash<size_t>{}(method.formal_params.size());
    }
};

// Класс
class Class : public Object {
    //std::unordered_set<Method, MethodHasher> methods_;
    std::string name_;
    const Class* parent_;
